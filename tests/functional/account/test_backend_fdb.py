...
class TestAccountBackend(BaseTestCase):
    def setUp(self):
        ...
        if os.path.exists(AccountBackendFdb.DEFAULT_FDB):
            ...
            fdb_file = str(Path.home())+'/.oio/fdb/fdb.cluster'
        self.account_conf = {
                'fdb_file': fdb_file}
    ...
    def test_refresh_bucket_with_sharded_ct(self):
        nb_obj_to_add = 5
        account_id = random_str(16)
        bucket = 'bucket-shards'
        cname = 'ct-sharded'
        cname_not_in_bucket = 'ct-not-in-bucket'
        cname_not_sharded = 'ct-not-sharded'
        account_id = random_str(16)
        ...
        test_shards = [{"index": 0, "lower": "", "upper": "1"},
                       {"index": 1, "lower": "1", "upper": "2"},
                       {"index": 2, "lower": "2", "upper": ""}]
        new_shards = self.container_sharding.format_shards(
            test_shards, are_new=True)
        _ = self.container_sharding.replace_shard(
            account_id, cname, new_shards, enable=True, reqid='sharding')

        self.wait_for_event('oio-preserved',
                            reqid="sharding",
                            timeout=3.0,
                            types=(EventTypes.CONTAINER_NEW,))
        # FIXME replace by waiting right events
        sleep(4)
