...
from oio.account.prom_metrics import Metric
...
class AccountBackendFdb():
    ...
    def _read_field_metrics(self, tr, field):
        metrics_field = dict()
        ct_metrics_space = fdb.Subspace(('metrics:', field))
        ct_metrics_range = ct_metrics_space.range()
        ...
        for key, value in iterator:
            ...
            metrics_field[region][storage_class] = int.from_bytes(
                                                        value,
                                                        byteorder='little')
    ...
    def _update_ct_stats_policy(self, tr, cname, region, objs_per_policy,
        ...
        c_space = fdb.Subspace(('metrics:', cname, region))
        ...
        reg_space = fdb.Subspace(('metrics:', cname, region))
        ...
        for key, val in res:
            ...
            if pol not in objs_per_policy:
                deltas[pol] = {}
                deltas[pol][field] = - int.from_bytes(val, byteorder='little')
