...
class AccountBackendFdb():
    ...
    def get_container_info(self, account_id, cname, **kwargs):
        ...
        bname = self._val_element(self.db, ct_space, 'bucket')

        if bname is not None:
            _bucket = fdb.Subspace((self._bucket_prefix, bname))
            rep_enabled = self._val_element(self.db, _bucket,
                                            'replication_enabled')
            if rep_enabled is not None:
                replication_enabled = rep_enabled

        info['replication_enabled'] = replication_enabled
